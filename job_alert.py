...
def send_email(job_title, job_url):
    ...
    body = f"A matching role was found!\n\nRole: {job_title}\n\nView Post: {job_url}"
...
def scan_ats_platform(platform_domain, titles_query, locations_query, seen_jobs):
    """Scans Google for all open roles across a platform with no time limits."""
    search_query = f"site:{platform_domain} {titles_query} {locations_query}"
    encoded_query = urllib.parse.quote_plus(search_query)
    
    # NO TIME LIMITS: The &tbs time filter has been completely removed
...
def main():
    print("🔄 Starting universal web-wide ATS scan (All Historical Posts)...")
    seen_jobs = load_seen_jobs()
    
    # Your target title clusters
    ...
    # Your location boundaries near 66221 + strict remote rules
    locations = '("66221" OR "Overland Park" OR "Olathe" OR "Leawood" OR "Lenexa" OR "Kansas City" OR ("remote" NOT "hybrid"))'
    
    # Complete list of major enterprise and startup platforms across the web
    ats_platforms = [
